...
// Parse a string to d amount of decimals to float
...
func askForName(appState *AppState) {
	prefs := appState.app.Preferences()

	if !fyne.CurrentDevice().IsMobile() {
		u, err := user.Current()
		if err == nil {
			if u.Username != "" {
				appState.user = u.Username
				appState.userLabel.SetText("This application is running for: " + u.Username)
			}
		} else {
		...
		dlg := dialog.NewForm("Παρακαλώ εισάγετε το ονοματεπώνυμο σας", "Save", "Cancel",
			[]*widget.FormItem{
				...
				if b && input.Text != "" {
					...
					name = trimmed
					appState.user = name
					appState.userLabel.SetText("This application is running for: " + name)
				} else {
					log.Println("Could not get the username...")
				}
			}, appState.window)
